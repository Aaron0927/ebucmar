...
void drive_machine(conn *c) {
    ...
    while (!stop) {
        ...
        switch(c->state) {
        case conn_listening://! 监听是否有新的连接
            addrlen = sizeof(addr);
            if ((sfd = accept(c->sfd, (struct sockaddr *)&addr, &addrlen)) == -1) {//! accept建立连接
                if (errno == EAGAIN || errno == EWOULDBLOCK) {
                    /* these are transient, so don't log anything */
                    stop = true;
                } else if (errno == EMFILE) { //! 溢出
                    if (settings.verbose > 0)
        ...
        case conn_read:
            ...
            case READ_NO_DATA_RECEIVED:
                conn_set_state(c, conn_waiting);
                break;
            case READ_DATA_RECEIVED: //! go to conn_parse_cmd
                conn_set_state(c, conn_parse_cmd);
                break;
            case READ_ERROR:
                conn_set_state(c, conn_closing);
...
int main (int argc, char **argv) {
    ...
    while (-1 != (c = getopt(argc, argv,
        ...
        case 'o': /* It's sub-opts time! */
            ...
            break;
		case 'z': /* init ramcube config file!*/
			ramcube_config_file = strdup(optarg);
			break;
        default:
	...
	/* this has to be done BEFORE thread_init() */
	if (ramcube_config_file)
		ramcube_adjust_memcached_settings(&settings);
	...
	/* RAMCube init */
	if (ramcube_config_file)
		ramcube_init(main_base, &settings);

    /* Drop privileges no longer needed */
    drop_privileges();

    /* enter the event loop 
     * ramcube starte then process go into here,loop loop ..... 
     */
